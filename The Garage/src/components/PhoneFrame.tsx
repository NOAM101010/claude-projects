...
function StatusBar() {
  const c = '#fff';
  return (
    <div style={{ display: 'flex', gap: 154, alignItems: 'center', justifyContent: 'center', padding: '21px 24px 19px', position: 'relative', zIndex: 20, width: '100%', boxSizing: 'border-box' }}>
...
export default function PhoneFrame({ children }: { children: ReactNode }) {
  const frame: CSSProperties = {
    ...
    background: '#000', boxShadow: '0 40px 80px rgba(0,0,0,0.18), 0 0 0 1px rgba(0,0,0,0.12)',
    fontFamily: '-apple-system, system-ui, sans-serif', WebkitFontSmoothing: 'antialiased',
  } as CSSProperties;
  return (
    <div style={{ position: 'relative', filter: 'drop-shadow(0 40px 90px rgba(0,0,0,.7))' }}>
      <div style={frame}>
