...
export function Auth() {
  const s = useStore();
  const L = s.L();
  const heroShot = s.photoCats.hero?.[0] || s.photoCats.exterior?.[0] || '/assets/a1-side.jpg';
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 30, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '0 30px 52px', animation: 'gFade .8s ease both' }}>
      <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${heroShot})`, backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.5 }} />
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg,rgba(8,9,11,.45) 0%,rgba(8,9,11,.86) 52%,#08090B 100%)' }} />
      <div style={{ position: 'relative', animation: 'gRise 1s cubic-bezier(.16,.84,.24,1) both' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 30 }}>
          {L.authOptions.map((label, n) => (
            <button key={n} onClick={s.enterApp} style={{
