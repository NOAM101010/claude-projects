...
**סבב 4 (Q1-Q7) הושלם במלואו, אומת (tsc/build/test:all ירוקים לכל שלב), Q5+Q6 עברו reviewer, נדחף ל-`origin/main`** (קומיטים `2997226`→`dbbdeac`→`15b1274`). התוכנית המלאה שמורה ב-`C:\Users\noam7\.claude\plans\swift-snuggling-harp.md`. **ממתין לבדיקה חיה של המשתמש (2-דפדפנים ל-Q5/Q6, ויזואלית ל-Q2/Q3/Q4) + הרצת SQL מרוכז.**
...
- **Q5 — דליפת קוסמטיקה במולטיפלייר (הכי גדול, reviewer אישר).** `BjSeat`/`PokerSeat` קיבלו `chipSkin`/`cardFace`/`cardBack`, stamped מ-`profile.equipped` ב-join (אותו דפוס כמו title/nameColor). כל רינדור מושב-יריב בבלאק'ג'ק/פוקר/SnG קורא עכשיו מה-seat במקום מה-profile המקומי של הצופה — כל שחקן סוף-סוף רואה את הסקין האמיתי של היריב, לא את שלו-עצמו. Fallback לשחקנים/מושבים ישנים (`cf-classic`/`bk-crimson`/`ck-classic`).
- **Q6 — מסך סיום BJ-עם-חברים (reviewer אישר, אחרי סבב תיקון).** הוסר כפתור "עוד יד" + auto-advance-לאותה-סצנה. עכשיו: cash תמיד, ודואל רק כשנקבע `duel.winner` (סוף מאץ') → תוצאה גלויה 4.5ש → **כל לקוח** מנווט עצמאית `/room/:code`. דואל באמצע-מאץ' ממשיך כרגיל (host שולח openBetting אחרי 3.2ש). **תיקון קריטי מ-review:** נוסף effect שלישי "רשת ביטחון" (11ש) — אם המארח מתנתק באמצע דואל לפני שנקבע מנצח, כל לקוח מנווט ללובי בעצמו במקום להישאר תקוע לצמיתות (הכפתור הידני הישן שהיה מטפל בזה הוסר). `SessionSummary` (לוח תוצאות דואל) — הכפתור "עוד יד" הוסר, אבל הפאנל עצמו נגיש עכשיו דרך כפתור קבוע ליד `DuelBoard` (שיפור לעומת קודם, לא רק שימור).
- **Q7 — QA/ליטוש כללי.** תג "השולחן של X" הושלם גם בבלאק'ג'ק (היחיד שפוצל מ-Q3). הישגי `king_of_night`/`legend_of_night` + הטייטל `ttl_night_king` הבלתי-ניתן-להשגה נמחקו לגמרי (תלויים ב-`stats.nightWins` שקפוא מ-Q1) + הוסרה תצוגת nightWins מהפרופיל. מחרוזת hardcoded אחת ב-`VipBadge.tsx` עברה ל-i18n. `activeMiniGame` — נבדק, כבר לא קיים בקוד. i18n parity 969/969.
- **הרצה חיה:** לא בוצעה (רק tsc/build/test) — Q5/Q6 (MP timing/cosmetics) דורשים בדיקת 2-דפדפנים לפני שנחשב "גמור לגמרי".
...
**סבב 4 (Q1-Q7) הושלם במלואו בקוד ונדחף. מה שנשאר הוא בדיקה חיה של המשתמש בלבד:**
1. **הרצת SQL** — `RUN-THIS-NEXT.sql` (3 חלקים: מחיקת `ev_night_champion`+`king_of_night`+`legend_of_night`, סנכרון צבעי `nc_cream`/`nc_neon`).
2. **בדיקה ויזואלית (Q2/Q3/Q4)** — כרטיס חבילה, שולחן אישי בכל סוגי המשחק, 4 דרגות VIP.
3. **בדיקת 2-דפדפנים (Q1 רגרסיה, Q5, Q6)** — הכי חשוב: Q5 (כל שחקן רואה את הסקין האמיתי של היריב, לא את שלו) ו-Q6 (סיום יד/מאץ' → כולם חוזרים ללובי יחד תוך ~4.5ש, כולל תרחיש host מתנתק באמצע דואל).
...
- **סבב 4 הושלם בקוד, ממתין לבדיקה חיה** (SQL + ויזואלי + 2-דפדפנים — ראה "What's left").
- **Stage M לא אומת חי** ב-2 דפדפנים — BJ/דואל עם חברים, הכי חשוב לבדוק (עוד מסבב 3, לא קשור לסבב 4).
