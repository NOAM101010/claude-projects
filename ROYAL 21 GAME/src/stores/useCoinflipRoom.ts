...
import { createState, reduce, MAX_SEATS } from '@/games/coinflip/engine';
...
interface CfRoomState {
  ...
  send: (userId: string, action: CfAction) => Promise<boolean>;
  leave: (userId: string) => Promise<void>;
}

export const useCoinflipRoom = create<CfRoomState>()((set, get) => ({
  room: null,
  members: [],
  state: null,
  isHost: false,
  ...
  create: async (userId) => {
    ...
    const room = await roomsService.create(userId, 'coinflip');
    if (!room) {
      set({ status: 'idle' });
      return null;
    }
  ...
  connect: async (room, userId) => {
    ...
    const initial = isHost
      ? await coinflipService.initIfEmpty(room.id)
      : await coinflipService.loadState(room.id);
  ...
  send: async (userId, action) => {
    ...
    if (solo || !room) {
      ...
      set({ state: reduce(state, { ...action, userId } as CfAction) });
      return true;
    }
    if (isHost && state) {
      // The host applies its own intent immediately, then publishes it.
      const next = reduce(state, { ...action, userId } as CfAction);
      set({ state: next });
      await coinflipService.publish(room.id, next);
      return true;
    }
    return coinflipService.sendAction(room.id, userId, action);
  },
