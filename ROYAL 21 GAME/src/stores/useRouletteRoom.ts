...
import { createState, reduce, MAX_SEATS } from '@/games/roulette/engine';
...
interface RouletteRoomState {
  ...
  send: (userId: string, action: RouletteAction) => Promise<boolean>;
  leave: (userId: string) => Promise<void>;
}

export const useRouletteRoom = create<RouletteRoomState>()((set, get) => ({
  room: null,
  members: [],
  state: null,
  isHost: false,
  ...
  create: async (userId) => {
    ...
    const room = await roomsService.create(userId, 'roulette');
    if (!room) {
      set({ status: 'idle' });
      return null;
    }
  ...
  connect: async (room, userId) => {
    ...
    const initial = isHost
      ? await rouletteService.initIfEmpty(room.id)
      : await rouletteService.loadState(room.id);
    ...
    const startHostLoop = () => {
      ...
      const stopRun = rouletteService.runHost(
        room.id,
    ...
    cleanups.push(rouletteService.subscribeState(room.id, (next) => {
  ...
  send: async (userId, action) => {
    ...
    if (solo || !room) {
      ...
      set({ state: reduce(state, { ...action, userId } as RouletteAction) });
      return true;
    }
    if (isHost && state) {
      // The host applies its own intent immediately, then publishes it.
      const next = reduce(state, { ...action, userId } as RouletteAction);
      set({ state: next });
      await rouletteService.publish(room.id, next);
      return true;
    }
    return rouletteService.sendAction(room.id, userId, action);
  },
