...
function makeSeat(
  userId: string, username: string, avatar: AvatarConfig, level: number, seat: number, stack: number,
  ...
  return {
    userId, username, avatar, level, seat, stack, title, nameColor, chipSkin, cardFace, cardBack,
...
export function reduce(prev: PokerState, action: PokerAction): PokerState {
  ...
  switch (action.type) {
    case 'join': {
      ...
      const seat = makeSeat(
        action.userId, action.username, action.avatar, action.level, seatNo, buyIn,
