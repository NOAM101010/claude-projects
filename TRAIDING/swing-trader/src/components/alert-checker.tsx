  ...
  return (
    <div className="fixed bottom-4 inset-x-4 z-[120] flex flex-col items-center gap-2 pointer-events-none sm:inset-x-auto sm:end-4 sm:items-end">
      {toasts.map((t) => (
        <div
          key={t.id}
          className="glass rounded-2xl px-4 py-3 pointer-events-auto flex items-center gap-3 border border-[rgba(245,158,11,0.45)] shadow-[0_0_28px_-8px_rgba(245,158,11,0.5)] max-w-sm"
        >
          ...
          <div className="text-sm">
            ...
            <span className="text-[var(--fg-dim)]"> · נוכחי ${t.currentPrice.toFixed(2)}</span>
            {t.note && <div className="text-xs text-[var(--muted)] mt-0.5">{t.note}</div>}
          </div>
          <button
